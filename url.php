...
if(isset($_COOKIE["visitor_hash"])){
    ...
    $q_visit = $sql->query("SELECT * FROM visitors
                           WHERE hash = '".$cookie_hash."' ");
    ...
    if($sql->num($q_visit) > 0){
        ...
        // добавляем просмотр
        $add_visit = $sql->query(
          "INSERT INTO visits (id_link, id_visitor, ip, date_add)
          VALUE ('".$r_link['id']."', '".$id_visitor."',
          '".$_SERVER['REMOTE_ADDR']."', ".(time()).")"
          );

        $update = $sql->query(
          "UPDATE visitors SET last_change = '".(time())."'
          WHERE id = ".$id_visitor.""
        );
...
if($create == 1){
    ...
    $insert = $sql->query(
      "INSERT INTO visitors (hash, last_change, date_add)
      VALUE ('".$hash."', '".($time)."', '".($time)."')"
    );
    ...
    if($insert){
        ...
        if($sql->num($select) > 0){
            ...
            $update = $sql->query("UPDATE visitors SET hash = '".$reafl_hash."'
                                  WHERE id = ".$s_r['id']."");
