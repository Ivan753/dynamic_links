...
<?php
    ...
    if(isset($_REQUEST["period"])){
        switch($_REQUEST["period"]){
            case 'yesterday':
                $time_ago = 86400; $time_text= "вчера"; $period = 1;
            break;
            case 'month':
                $time_ago = 2678400; $time_interval = 32; $coef_per = 3600*24/4;
                $dis_date = "d.m"; $time_text= "месяц"; $period = 2;
            break;
            case 'half_year':
                $time_ago = 15811200; $time_interval = 184;
                $coef_per = 3600*24*31/10; $dis_date = "m.Y";
                $time_text= "полгода"; $period = 3;
            break;
        }
    }
...
?>
    <section class = "s_time_interval">
        <a href = "statistics?link=<?php echo $link ?>" onclick = "return nav.go(this, event)"><div class = "s_time_interval_butt <?php echo ($period==0)?'s_t_i_b_active':'' ?>">Сегодня</div></a>
        <a href = "statistics?link=<?php echo $link ?>&period=yesterday" onclick = "return nav.go(this, event)"><div class = "s_time_interval_butt <?php echo ($period==1)?'s_t_i_b_active':'' ?>">Вчера</div></a>
        <a href = "statistics?link=<?php echo $link ?>&period=month" onclick = "return nav.go(this, event)"><div class = "s_time_interval_butt <?php echo ($period==2)?'s_t_i_b_active':'' ?>">Месяц</div></a>
        <a href = "statistics?link=<?php echo $link ?>&period=half_year" onclick = "return nav.go(this, event)"><div class = "s_time_interval_butt <?php echo ($period==3)?'s_t_i_b_active':'' ?>">Полгода</div></a>
    </section>
 ...
 <?php
    ...
    $q_visits = $sql->query(
      "SELECT * FROM visits WHERE id_link = ".$links['id']." AND
      date_add >= ".$datetime_today." AND date_add < ".$datetime_today_end."
      ORDER BY date_add"
    );
    ...
    if($sql->num($q_visits) > 0){
        ...
        do{
            ...
            $q_new = $sql->query(
              "SELECT id FROM visits WHERE id_link = ".$links['id']." AND
              id_visitor = ".$visits['id_visitor']." AND
              date_add < ".$datetime_today." "
            );
            ...
            $dif = round(
              ($this_time - ($datetime_today + $last_item*$coef_per))/($coef_per)
            );
    ...
    <br>
    <div class = "dates"><span><?php echo date("d.m H:i", $datetime_today)?></span><span><?php echo date("d.m H:i", $datetime_today_end-1)?></span></div>
        <p>Все просмотры за <?php echo $time_text ?>: <span><?php echo $all_today ?></span></p>
        <p style = "color:#898900">Новые посетители за <?php echo $time_text ?>: <span><?php echo $new_today ?></span></p>
        <p style = "color:#125656">Всего посещений: <span><?php echo $all_visit ?></span></p>
        </div>
    </section>
