 ...
 * Операция является неприоритетной, поэтому при возникновении непридвиденной
 * ошибки переходим к созданию ключа
...
if($sql->num($_free) == 1){
    ...
    $update = $sql->query(
			"UPDATE links SET id_user = '".$row['id']."', content = '".$link."',
			type = 1, flag = 1  WHERE id = ".$free_r['id'].""
		);
...
// id в 62-ричной системе и случайной заглавной буквы вначале

$hash = fake_hash(10);

$insert = $sql->query(
	"INSERT INTO links (id_user, url, content, type)
	VALUE ('".$row['id']."', '".$hash."', '".$link."', 1)"
);

if($insert){

    $select = $sql->query(
			"SELECT * FROM links WHERE id_user = '".$row['id']."' AND url = '".$hash."'"
		);
    ...
    if($sql->num($select) > 0){
        ...
        $update = $sql->query(
					"UPDATE links SET url = '".$url."' WHERE id = ".$s_r['id'].""
				);
