...
$select = $sql->query("SELECT * FROM links WHERE id_user = ".$row['id']." AND id = ".$id_link."");

if($sql->num($select) == 1){
    
    // подтверждение удаления
    if(!isset($_SESSION["delete_link"]) OR $_SESSION["delete_link"] != md5($row['id'].$id_link)){
    ...
    $delete_visit = $sql->query("DELETE FROM visits WHERE id_link = ".$id_link." ");
    
    if($delete_visit){
        
        $delete = $sql->query("UPDATE links SET flag = 0, id_user = 0, content = '0' WHERE id = ".$id_link." ");
